...
void CADecoderDynamoRIO::decode(uint64_t addr, uint8_t* ins, size_t size)
{
	nbytes = size;
	ins_addr = addr;
	for (size_t i =0; i<nbytes; i++){
...
size_t CADecoderDynamoRIO::CallToPush(uint8_t *payload)
{
	...
	if (instr_reads_memory(&instr))
	{
		...
		if (instr_is_call_indirect(&instr))
		{
			...
			new_size = sizeof(abs_addr)+1;
		}
	}
	
	//Check if need to depie the push instruction
	this->decode(this->ins_addr, payload, new_size);
	if (this->needDepie()){
		new_size = this->depie(payload);
	}
...
size_t CADecoderDynamoRIO::JmpToPush(uint8_t *payload, bool inPLT)
{
	 ...
	 * If it is a indirect jmp in plt, then no need to do that, since red zone
	 * do not allow any function call inside
	 */
	...
	if (instr_reads_memory(&instr))
	{
		memcpy(payload+offset, raw_byte, nbytes);
		if (payload[offset] == 0xff)
			payload[offset+1] += 0x10;
		else
			payload[offset+2] += 0x10;
		new_size = nbytes;
	}
	else{
		if (nbytes == 2)
			payload[offset] = raw_byte[1] - 0x90;
		else{
			payload[offset] = raw_byte[0];
			payload[offset+1] = raw_byte[2] - 0x90;
		}
		new_size = nbytes-1;
	}

	//Check if need to depie the push instruction
	this->decode(this->ins_addr, push_ins, new_size);
	if (this->needDepie()){
		new_size = this->depie(push_ins);
	}

...
//CHEN MODIFIED
/*
   inject the code into binary
   return the virtual address it is injected to, so the user can 
   adjust the trampline jmp offset.
   Size will be page aligned to optimize the cache performance
   (word aligned should be enough, here just for safty)
 */
void* BPatch_binaryEdit::injectCode(u_int size, void* payload)
{
	Address ret;
	u_int aligned_size = (size & 0xfffffffffffff000)+0x1000;
	
	ret = origBinEdit->inferiorMalloc(aligned_size);
	origBinEdit->writeTextSpace((void*)ret,size,payload);
...
//CHEN NOTE
//Here we can install our own sigaction handler

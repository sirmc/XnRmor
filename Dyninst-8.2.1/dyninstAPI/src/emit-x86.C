...
bool EmitterAMD64Stat::emitPLTCall(func_instance *callee, codeGen &gen) {
   ...
   *(unsigned int*)insn = dest - (gen.currAddr() + sizeof(unsigned int) + 2);
   ...
   /*CHEN change RIP relative into absulote call 
   *insn++ = 0xFF;
   *insn++ = 0x14;
   *insn++ = 0x25;
   *(unsigned int*)insn = dest;
   CHEN done*/
   insn += sizeof(unsigned int);
   SET_PTR(insn, gen);
   return true;
...
//CHEN NOTE
/*
   Here, try to mimic a trap to put the external lib function for instrumentation
   If found, return it, otherwise invoke inferiorMalloc to get one
 */

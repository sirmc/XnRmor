...
//CHEN NOTE
/*
   Here is the chance to do a rewrite without going into 
   Dyninst insane class system.
   1. InferiorMalloc a space
   2. Adjust the trampline offset in orignal code space via parseAPI
   3. Use writeTextSpace to write into rewrited binary.
    
   Internal:
   1. A user can ask a "heap" location via inferiorMalloc interface.
      It will first find a suitable place change the watermark.
      Then a memorytracker is insert into the memorytracker_ RB tree.
   2. User land stuff, similar to what dyninst do
   3. Get the internal map from the memorytraker_, and memcpy the payload in place
*/
...
//CHEN NOTE
/* 
   Dyninst use this function to allocated memory. 
   The instrumented data/function are stays here. 
   Each external lib function (instrumentation handler) are treated as a heap alone. 
   But for the .text, .init etc. They are treat as a whole heap, and the caller need 
   to maintain this region
 */
